...
def test_parse_first():
    resp, new_buf = parse.parse_start("Welcome to Minesweeper. BoardResp: 40 columns by 37 rows. Players: 1 including you. Type 'help' for help.\n", first=True)
    assert isinstance(resp, parse.HelloResp)
...
def test_parse_boom():
    ...
    assert isinstance(resp, parse.BoomResp)
    assert new_buf == ''
    resp, new_buf = parse.parse_start("BOOM!\r\n", (1,1))
    assert isinstance(resp, parse.BoomResp)
    assert new_buf == ''
...
def test_parse_board():
    ...
    assert isinstance(resp, parse.BoardResp)
    assert new_buf == ''
...
def test_parse_help():
    ...
    assert isinstance(resp, parse.HelpResp)
    assert new_buf == ''
